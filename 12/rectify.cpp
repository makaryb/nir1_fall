...
void fundamental(int l0, int l1, int l2, int r0, int r1, int r2) {
  ...
  for (auto it = llines.begin(); it != llines.end(); it++) {
    ...
    b = (*it)[1];
    c = (*it)[2];
    cv::line(lcolor, cv::Point(0, -c/b), cv::Point(lcolor.cols-1, -(a/b*(lcolor.cols-1)+c/b)), cv::Scalar::all(255));
  }
  for (auto it = rlines.begin(); it != rlines.end(); it++) {
    a = (*it)[0];
    b = (*it)[1];
    c = (*it)[2];
    cv::line(rcolor, cv::Point(0, -c/b), cv::Point(rcolor.cols-1, -(a/b*(rcolor.cols-1)+c/b)), cv::Scalar::all(255));
  }
...
void calib(void) {
  ...
  if (argvflg&1) writer.open("result.avi", cv::VideoWriter::fourcc('M','J','P','G'), 30, cv::Size(XX, YY));
  if (argvflg&1) for (int i = 0; i < 10; i++) disp_and_write(Right, writer);
  if (argvflg&1) for (int i = 0; i < 10; i++) disp_and_write(Left, writer);
  if (argvflg&1) for (int i = 0; i < 10; i++) disp_and_write(Right, writer);
  if (argvflg&1) for (int i = 0; i < 10; i++) disp_and_write(Left, writer);
  if (argvflg&1) for (int i = 0; i < 10; i++) disp_and_write(Right, writer);
  if (argvflg&1) for (int i = 0; i < 10; i++) disp_and_write(Left, writer);
  ...
  if (argvflg&1) disp_and_write(Both, writer);
  else { disp_mode = Both; display(); }
  ...
  for (int mul = 512; mul >= 8; mul /= 2) {
    ...
    int step = 0;
    for ( ; ; ) {
           if (step ==  0) { l0+=mul; r0-=mul; }
      else if (step ==  1) { l0-=mul; r0+=mul; }
      else if (step ==  2) { l1+=mul; r1-=mul; }
      else if (step ==  3) { l1-=mul; r1+=mul; }
      else if (step ==  4) { l2+=mul; r2-=mul; }
      else if (step ==  5) { l2-=mul; r2+=mul; }
      else if (step ==  6) l0+=mul;
      else if (step ==  7) r0-=mul;
      else if (step ==  8) l0-=mul;
      else if (step ==  9) r0+=mul;
      else if (step == 10) l2+=mul;
      else if (step == 11) r2-=mul;
      else if (step == 12) l2-=mul;
      else if (step == 13) r2+=mul;
      else if (step == 14) l1+=mul;
      else if (step == 15) r1-=mul;
      else if (step == 16) l1-=mul;
      else if (step == 17) r1+=mul;
      else {}
      rotate(l0, l1, l2, r0, r1, r2);
      if (argvflg&1) disp_and_write(Both, writer);
      else { disp_mode = Both; display(); }
  ...
  if (argvflg&2) cbout("l0.out" , 50, l0, l1, l2, r0, r1, r2, 10,  0,  0,  0,  0,  0);
  if (argvflg&2) cbout("l1.out" , 50, l0, l1, l2, r0, r1, r2,  0, 10,  0,  0,  0,  0);
  if (argvflg&2) cbout("l2.out" , 50, l0, l1, l2, r0, r1, r2,  0,  0, 10,  0,  0,  0);
  if (argvflg&2) cbout("r0.out" , 50, l0, l1, l2, r0, r1, r2,  0,  0,  0, 10,  0,  0);
  if (argvflg&2) cbout("r1.out" , 50, l0, l1, l2, r0, r1, r2,  0,  0,  0,  0, 10,  0);
  if (argvflg&2) cbout("r2.out" , 50, l0, l1, l2, r0, r1, r2,  0,  0,  0,  0,  0, 10);
  if (argvflg&2) cbout("l01.out", 50, l0, l1, l2, r0, r1, r2, 10, 10,  0,  0,  0,  0);
  if (argvflg&2) cbout("l12.out", 50, l0, l1, l2, r0, r1, r2,  0, 10, 10,  0,  0,  0);
  if (argvflg&2) cbout("l20.out", 50, l0, l1, l2, r0, r1, r2, 10,  0, 10,  0,  0,  0);
  if (argvflg&2) cbout("r01.out", 50, l0, l1, l2, r0, r1, r2,  0,  0,  0, 10, 10,  0);
  if (argvflg&2) cbout("r12.out", 50, l0, l1, l2, r0, r1, r2,  0,  0,  0,  0, 10, 10);
  if (argvflg&2) cbout("r20.out", 50, l0, l1, l2, r0, r1, r2,  0,  0,  0, 10,  0, 10);
  /*-----------------------*/
  DDDD = 1;
  ...
  SIZE_SHW = SIZE_S*SIZE_H*SIZE_W;
  rotate(l0, l1, l2, r0, r1, r2);
  if (argvflg&4) cv::imwrite("rec_R.png", rrframe);
  if (argvflg&4) cv::imwrite("rec_L.png", llframe);
  if (argvflg&1) for (int i = 0; i < 10; i++) disp_and_write(Right, writer);
  if (argvflg&1) for (int i = 0; i < 10; i++) disp_and_write(Left, writer);
  if (argvflg&1) for (int i = 0; i < 10; i++) disp_and_write(Right, writer);
  if (argvflg&1) for (int i = 0; i < 10; i++) disp_and_write(Left, writer);
  ...
  if (argvflg&1) for (int i = 0; i < 200; i++) {
  ...
  if (argvflg&8) fundamental(l0, l1, l2, r0, r1, r2);
...
int main(int argc, char** argv) {
  printf("OpenCV-%s will be used.\n", CV_VERSION);
  if ((argc != 9)&&(argc != 10)) {
    printf("Usage: './program_name right.ppm left.ppm max_disp min_disp penalty inhibit divisor argvflg'\n");
    printf("Usage: './program_name right.ppm left.ppm max_disp min_disp penalty inhibit divisor argvflg LOOP'\n");
    return 1;
  }
